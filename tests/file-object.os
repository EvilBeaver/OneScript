...
Функция ДатаСТочностьюДоСекунды(Знач Дата)
	Возврат Дата(Формат(Дата, "ДФ=ггггММддЧЧммсс"));
КонецФункции

// Время файла должно попасть в интервал записи. Сравнивать с моментом до записи с допуском нельзя:
// на медленной машине CI запись файла занимает секунды. Запас в секунду - на округление до секунд
// и на шаг времени в 2 секунды у FAT32
Процедура ПроверитьДатуВИнтервале(Знач Дата, Знач Начало, Знач Конец, Знач ЧтоПроверяем)
	Дата = ДатаСТочностьюДоСекунды(Дата);
	юТест.ПроверитьБольшеИлиРавно(Дата, ДатаСТочностьюДоСекунды(Начало) - 1, ЧтоПроверяем + " раньше записи");
	юТест.ПроверитьМеньшеИлиРавно(Дата, ДатаСТочностьюДоСекунды(Конец) + 1, ЧтоПроверяем + " позже записи");
КонецПроцедуры
...
Процедура ТестДолжен_ПроверитьПолучениеДатСозданияИИзменения() Экспорт	
	...
	ДоЗаписи = ТекущаяДата();
	ЗаписьТекста = Новый ЗаписьТекста(ВремФайл);
	ЗаписьТекста.ЗаписатьСтроку("---");
	ЗаписьТекста.Закрыть();	
	ПослеЗаписи = ТекущаяДата();
	Файл = Новый Файл(ВремФайл);
	ВремяСоздания = Файл.ПолучитьВремяСоздания();

	ПроверитьДатуВИнтервале(ВремяСоздания, ДоЗаписи, ПослеЗаписи, "Время создания");
	...
	ПроверитьДатуВИнтервале(ВремяИзменения, ДоЗаписи, ПослеЗаписи, "Время изменения");
