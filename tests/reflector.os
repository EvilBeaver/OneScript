...
Процедура ТестДолжен_ПроверитьИзвестныеТипы() Экспорт
	...
	Для Каждого ИзвестныйТип Из ИзвестныеТипы Цикл
		юТест.ПроверитьТип(ИзвестныйТип, "Тип");
	КонецЦикла;

	юТест.ПроверитьИстину(ИзвестныеТипы.Найти(Тип("Массив")) > 0);
	юТест.ПроверитьИстину(ИзвестныеТипы.Найти(Тип("Структура")) > 0);
	юТест.ПроверитьИстину(ИзвестныеТипы.Найти(Тип("Число")) > 0);
	юТест.ПроверитьИстину(ИзвестныеТипы.Найти(Тип("Рефлектор")) > 0);
	юТест.ПроверитьИстину(ИзвестныеТипы.Найти(Тип("МойКлассныйТип")) > 0);
