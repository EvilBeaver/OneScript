...
Процедура ТестДолжен_ПроверитьМетод_ПолучитьТаблицуМетодов_ПроверитьЗначенияПоУмолчанию() Экспорт
	...
	юТест.Проверить(Параметры[0].ЗначениеПоУмолчанию = Неопределено, "Значение по умолчанию = Отсутсвует");
	юТест.Проверить(Параметры[1].ЗначениеПоУмолчанию = Неопределено, "Значение по умолчанию = Неопределено");
	юТест.Проверить(Параметры[2].ЗначениеПоУмолчанию = Истина, "Значение по умолчанию = Булево");
	юТест.Проверить(Параметры[3].ЗначениеПоУмолчанию = 1, "Значение по умолчанию = 1");
	юТест.Проверить(Параметры[4].ЗначениеПоУмолчанию = "Строка", "Значение по умолчанию = Строка");
КонецПроцедуры