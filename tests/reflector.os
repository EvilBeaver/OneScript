...
Процедура ТестДолжен_ПроверитьИзвестныеТипы() Экспорт
	...
	юТест.ПроверитьТип(ИзвестныеТипы, "Массив");
	юТест.ПроверитьНеРавенство(ИзвестныеТипы.Количество(), 0);

	ЕстьЧисло = Ложь;
	ЕстьМассив = Ложь;
	ЕстьМойКлассныйТип = Ложь;

	Для Каждого ИзвестныйТип Из ИзвестныеТипы Цикл

		юТест.ПроверитьТип(ИзвестныйТип, "Структура");

		юТест.ПроверитьИстину(ИзвестныйТип.Свойство("Тип"));
		юТест.ПроверитьИстину(ИзвестныйТип.Свойство("Имя"));
		юТест.ПроверитьИстину(ИзвестныйТип.Свойство("Примитивный"));
		юТест.ПроверитьИстину(ИзвестныйТип.Свойство("Пользовательский"));

		юТест.ПроверитьТип(ИзвестныйТип.Тип, "Тип");
		...
		юТест.ПроверитьЗаполненность(ИзвестныйТип.Тип);
		юТест.ПроверитьЗаполненность(ИзвестныйТип.Имя);

		Если ИзвестныйТип.Тип = Тип("Число") Тогда
			юТест.ПроверитьИстину(ИзвестныйТип.Примитивный);
			юТест.ПроверитьЛожь(ИзвестныйТип.Пользовательский);
			ЕстьЧисло = Истина;
		КонецЕсли;

		Если ИзвестныйТип.Тип = Тип("Массив") Тогда
			юТест.ПроверитьЛожь(ИзвестныйТип.Примитивный);
			юТест.ПроверитьЛожь(ИзвестныйТип.Пользовательский);
			ЕстьМассив = Истина;
		КонецЕсли;

		Если ИзвестныйТип.Тип = Тип("МойКлассныйТип") Тогда
			юТест.ПроверитьЛожь(ИзвестныйТип.Примитивный);
			юТест.ПроверитьИстину(ИзвестныйТип.Пользовательский);
			ЕстьМойКлассныйТип = Истина;
		КонецЕсли;

	КонецЦикла;

	юТест.ПроверитьИстину(ЕстьЧисло И ЕстьМассив И ЕстьМойКлассныйТип);
