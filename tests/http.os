...
Перем мАдресРесурса; // Базовый URL ресурса для HTTP-тестов (автовыбор доступного хоста)
...
////////////////////////////////////////////////////////////////////////////////
/// ВСПОМОГАТЕЛЬНЫЕ ФУНКЦИИ ВЫБОРА ХОСТА
////////////////////////////////////////////////////////////////////////////////

Функция ПроверитьДоступность(Хост)
	Попытка
	...
	Исключение
		Сообщить("HTTP тесты: недоступен хост " + Хост + " (" + ОписаниеОшибки() + ")");
		Возврат Ложь;
	КонецПопытки;
КонецФункции
...
Функция ВыбратьДоступныйХост()
	...
	Если ЗначениеИзОкружения <> Неопределено И ЗначениеИзОкружения <> "" Тогда
		Кандидаты.Добавить(ЗначениеИзОкружения);
	КонецЕсли;
	// Базовые кандидаты по умолчанию
	...
	Для Каждого Хост Из Кандидаты Цикл
		Если ПроверитьДоступность(Хост) Тогда
			Сообщить("HTTP тесты: выбран доступный хост " + Хост);
			Возврат Хост;
		КонецЕсли;
	КонецЦикла;

	Сообщить("HTTP тесты: ни один из кандидатных хостов не доступен. Используется https://httpbin.org по умолчанию.");
