...
Функция ПолучитьСписокТестов(ЮнитТестирование) Экспорт
	...
	Если Найти(СИ.ВерсияОС, "Windows") > 0 Тогда
		...
		ВсеТесты.Добавить("ТестДолжен_ПрочитатьВыводБезКонцевогоПереводаСтроки");
...
Процедура ТестДолжен_ПрочитатьВыводOscriptСразу() Экспорт
	...
	// Прочитать() возвращает вывод как есть, включая завершающий перевод строки
...
Процедура ТестДолжен_ПрочитатьВыводБезКонцевогоПереводаСтроки() Экспорт

	ИмяСкрипта = юТест.ИмяВременногоФайла("os");
	ЗаписьТекста = Новый ЗаписьТекста(ИмяСкрипта, КодировкаТекста.UTF8);
	ЗаписьТекста.ЗаписатьСтроку("Консоль.ВывестиСтроку(""line1"");");
	ЗаписьТекста.Записать("Консоль.Вывести(""line2"");");
	ЗаписьТекста.Закрыть();

	Процесс = СоздатьПроцесс(ПутьОСкрипт() + " """ + ИмяСкрипта + """",, Истина);
	Процесс.Запустить();
	Процесс.ОжидатьЗавершения();

	Стр = НормализоватьПереводыСтрок(Процесс.ПотокВывода.Прочитать());

	юТест.ПроверитьРавенство("line1" + Символы.ПС + "line2", Стр);
