...
Процедура ТестДолжен_ПроверитьФинальнуюЗапятуюВОбъявленииМетода() Экспорт
	...
	Исключение
		...
		юТест.ПроверитьНеравенство(0, Найти(Текст, "Ожидается идентификатор"));
		Возврат;
	КонецПопытки;
...
Процедура ТестДолжен_ПроверитьМинусВЗначенииПарамераПоУмолчанию() Экспорт
	...
	ЗагрузитьСценарийИзСтроки("Процедура Метод( Парам = -1 ) КонецПроцедуры");
	ЗагрузитьСценарийИзСтроки("Процедура Метод( Парам = - //перевод строки" " 1 ) КонецПроцедуры");
	ЗагрузитьСценарийИзСтроки("Процедура Метод( Парам = - Неопределено ) КонецПроцедуры");
	ЗагрузитьСценарийИзСтроки("Процедура Метод( Парам = - Null ) КонецПроцедуры");

	Ошибка = "Ожидается числовая константа";
