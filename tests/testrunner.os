...
Функция ИсключениеНеравенстваСтрок(ПервоеЗначение, ВтороеЗначение)
	...
	Для Сч = 1 По Мин(СтрДлина(ПервоеЗначение), СтрДлина(ВтороеЗначение)) Цикл
		...
		Если Символ1 <> Символ2 Тогда
			...
			НачалоПервой = Сред(ПервоеЗначение, ОтступНачала, ДиапазонОтНачала);
			НачалоВторой = Сред(ВтороеЗначение, ОтступНачала, ДиапазонОтНачала); 
			
			Префикс = 9; // Строка Х:
			
			Возврат "Различия в позиции " + ИндексРазличия + ".
			|Строка 1:" + НачалоПервой + СтрЗаменить(Сред(ПервоеЗначение, ОтступНачала+ДиапазонОтНачала, ДиапазонВКонце), Символы.ПС, "\n") + "
			|         " + Префикс + Лев(СтрокаНабора, СтрДлина(НачалоПервой)) + "^
			|Строка 2:" + НачалоВторой + СтрЗаменить(Сред(ВтороеЗначение, ОтступНачала+ДиапазонОтНачала, ДиапазонВКонце), Символы.ПС, "\n");
		КонецЕсли;
	КонецЦикла;
