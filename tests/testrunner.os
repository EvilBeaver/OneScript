...
Функция ИсключениеНеравенстваСтрок(ПервоеЗначение, ВтороеЗначение)
	...
	Для Сч = 1 По Мин(СтрДлина(ПервоеЗначение), СтрДлина(ВтороеЗначение)) Цикл
		...
		Если Символ1 <> Символ2 Тогда
			ИндексРазличия = Сч;
			ОтступНачала = ИндексРазличия - 5;
			Если ОтступНачала < 0 Тогда
				ОтступНачала = 1;
			КонецЕсли;
			
			НачалоПервой = Сред(ПервоеЗначение, ОтступНачала, 5);
			НачалоВторой = Сред(ВтороеЗначение, ОтступНачала, 5); 
			
			Возврат "Различия в позиции " + ИндексРазличия + ".
			|Строка 1:" + НачалоПервой + СтрЗаменить(Сред(ПервоеЗначение, ОтступНачала+5, 5), Символы.ПС, "\n") + "
			|              ^
			|Строка 2:" + НачалоВторой + СтрЗаменить(Сред(ВтороеЗначение, ОтступНачала+5, 5), Символы.ПС, "\n");
		КонецЕсли;
	КонецЦикла;
