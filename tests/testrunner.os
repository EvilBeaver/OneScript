...
Перем	_ПутьФайлаОтчетаТестированияВФорматеJUnitXML;
...
Процедура СоздатьСтруктуруПараметровЗапуска()
	...
	//СтруктураПараметровЗапуска.Вставить("ПутьФайлаОтчетаТестированияВФорматеJUnitXML", НРег("-reportpath"));
КонецПроцедуры
...
Функция ЗагрузитьТесты()
	...
	Для Каждого ПутьТеста Из Пути Цикл
		...
		Иначе
			...
			Исключение
				Сообщить("Не удалось загрузить тест "+ПолноеИмяТестовогоСлучая);
				Рез = Ложь;
...
Процедура ЗавершитьЗаписьВФайлОтчетаТестированияВФорматеJUnitXML(ЗаписьXML, ДатаНачала)
	...
	Для КлючСоответствия = 0 По СписокОшибок.Количество() - 1 Цикл
		ЭлементСписка = СписокОшибок[КлючСоответствия];
		
		// КлючСоответствия = ЭлементСписка.УИДСтрокиДерева; //ЭлементСписка.Значение["УИДСтрокиДерева"];
		КэшОшибок.Вставить(КлючСоответствия, ЭлементСписка);
	КонецЦикла;
	...
	Для Сч = 0 По НаборТестов.Количество() - 1 Цикл
		...
		ЗаписьXML.ЗаписатьАтрибут("name", СтрЗаменить(ОписаниеТеста.ПолноеИмя, ТекущийКаталог()+"\", "")); //XMLСтрока(ОписаниеТеста.ИмяКласса));
		ЗаписьXML.ЗаписатьНачалоЭлемента("properties");	
		ЗаписьXML.ЗаписатьКонецЭлемента();

		ЗаполнитьРезультатТестовогоСлучая(ЗаписьXML, ОписаниеТеста, Сч, КэшОшибок);
	...
	ФайлТеста = Новый Файл(ОписаниеТеста.ПолноеИмя);
...
Процедура ЗаполнитьРезультатТестовогоСлучая(ЗаписьXML, ОписаниеТеста, Сч, КэшОшибок)
		
	ЗаписьXML.ЗаписатьНачалоЭлемента("testcase");
	ЗаписьXML.ЗаписатьАтрибут("classname", СтрЗаменить(ОписаниеТеста.ПолноеИмя, ТекущийКаталог()+"\", "")); //ОписаниеТеста.ПолноеИмя);
	...
	КлючСоответствия	= Сч; //СтрокаТестов._guid;
...
Функция ПутьФайлаОтчетаТестированияВФорматеJUnitXML()
	Возврат ТекущийКаталог(); //_ПутьФайлаОтчетаТестированияВФорматеJUnitXML;
КонецФункции
...
Функция ВыполнитьТест(ОписаниеТеста, Сч)
	...
	Успешно = ВыполнитьПроцедуруТестовогоСлучая(Тест, "ПередЗапускомТеста", ИмяМетода, ОписаниеТеста, Сч);
	...
	Иначе
		...
		Исключение
			...
			Иначе
				Рез = ВывестиОшибкуВыполненияТеста("", ОписаниеТеста, Сч, текстОшибки, ИнфоОшибки);
				// ВывестиОшибку("Упал тест <"+ИмяМетода+">, файл <"+ОписаниеТеста.ПолноеИмя+">: "+Символы.ПС+"    "+текстОшибки);
				// Рез = ЗначенияСостоянияТестов.Сломался;
			КонецЕсли;
		КонецПопытки;
		
		Успешно = ВыполнитьПроцедуруТестовогоСлучая(Тест, "ПослеЗапускаТеста", ИмяМетода, ОписаниеТеста, Сч);
		Если Не Успешно Тогда
			Рез = ЗначенияСостоянияТестов.Сломался;
		КонецЕсли;
	КонецЕсли;
...
Функция ВывестиОшибкуВыполненияТеста(ПредставлениеОшибки, ОписаниеТеста, Сч, текстОшибки, ИнфоОшибки)
	ИмяМетода = ОписаниеТеста.ИмяМетода;
	...
	Возврат ЗначенияСостоянияТестов.Сломался; //СостояниеВыполнения;
КонецФункции

Функция ВыполнитьПроцедуруТестовогоСлучая(Тест, ИмяПроцедуры, ИмяТестовогоСлучая, ОписаниеТеста, Сч)
	Успешно = Ложь;
	...
	Исключение
		...
		Если ЕстьОшибка_МетодОбъектаНеОбнаружен(текстОшибки, ИмяПроцедуры) Тогда
			Успешно = Истина;
		Иначе
			Рез = ВывестиОшибкуВыполненияТеста("Упал метод "+ИмяПроцедуры, ОписаниеТеста, Сч, текстОшибки, ИнфоОшибки);
