...
Процедура ЗавершитьЗаписьВФайлОтчетаТестированияВФорматеJUnitXML(ЗаписьXML, ДатаНачала)
	...
	// ЗаписьXML.ЗаписатьАтрибут("disabled", XMLСтрока(КоличествоНереализованныхТестов));
	ЗаписьXML.ЗаписатьАтрибут("skipped", XMLСтрока(КоличествоНереализованныхТестов));

	ЗаписьXML.ЗаписатьНачалоЭлемента("testsuite");	
	ЗаписьXML.ЗаписатьАтрибут("name", СтрЗаменить(ТекущийСценарий().Источник, ТекущийКаталог()+"\", ""));
