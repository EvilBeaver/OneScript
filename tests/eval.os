...
Процедура ТестДолжен_ПроверитьEvalНаРазныхЭкземплярахThis() Экспорт
	
	ПутьСценарий = ОбъединитьПути(ТекущийСценарий().Каталог, "testdata", "evalDifferentThis.os");
	Первый = ЗагрузитьСценарий(ПутьСценарий);
	Второй = ЗагрузитьСценарий(ПутьСценарий);
	
	Первый.УстановитьЗначение("Первый");
	Второй.УстановитьЗначение("Второй");
	
	Рез1 = Первый.ПолучитьЧерезВычислить();
	Рез2 = Второй.ПолучитьЧерезВычислить();
	
	юТест.ПроверитьРавенство("Первый", Рез1);
	юТест.ПроверитьРавенство("Второй", Рез2);
...
Процедура ТестДолжен_ПроверитьВызовыВыполнитьСПопытками() Экспорт
    ...
    юТест.ПроверитьРавенство("0134567+", Рез);
   
КонецПроцедуры

Процедура ТестДолжен_ПроверитьРаботоспособностьВПодключенныхСценариях() Экспорт
    
    Библиотека = ОбъединитьПути(ТекущийСценарий().Каталог, "eval-in-libraries");
    
    ПодключитьСценарий(Библиотека + "/src/Тесты/Тест.os", "ТестEvalInLibraries");
    
    А = Новый ТестEvalInLibraries();
    А.Тест();
