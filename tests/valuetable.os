...
Функция ПолучитьСписокТестов(ЮнитТестирование) Экспорт
	...
	ВсеТесты.Добавить("ТестДолжен_НайтиИспользуетИндекс");
	ВсеТесты.Добавить("ТестДолжен_НайтиВозвращаетНеопределеноЕслиНетСовпадений");
	ВсеТесты.Добавить("ТестДолжен_НайтиПоНесколькимКолонкам");
	ВсеТесты.Добавить("ТестДолжен_НайтиПоНесколькимКолонкамСЧастичнымИндексом");
	ВсеТесты.Добавить("ТестДолжен_НайтиСИндексомБыстрееЧемБезИндекса");
...
Процедура ТестДолжен_НайтиИспользуетИндекс() Экспорт
	...
	юТест.ПроверитьЛожь(НайденнаяСтрока = Неопределено, "Строка должна быть найдена по проиндексированной колонке");
	юТест.ПроверитьРавенство(НайденнаяСтрока.СИндексом, ИскомыйКлюч, "Найденная строка должна содержать искомое значение");
	...
	юТест.ПроверитьИстину(НайденнаяСтрока = Неопределено, "Несуществующее значение не должно быть найдено по проиндексированной колонке");
...
Процедура ТестДолжен_НайтиВозвращаетНеопределеноЕслиНетСовпадений() Экспорт
	...
	юТест.ПроверитьИстину(НайденнаяСтрока = Неопределено, "Должно вернуть Неопределено при отсутствии совпадений");
...
Процедура ТестДолжен_НайтиПоНесколькимКолонкам() Экспорт
	...
	юТест.ПроверитьЛожь(НайденнаяСтрока = Неопределено, "Строка должна быть найдена по первой из колонок");

	НайденнаяСтрока = ТЗ.Найти("ЗначениеБ", "А, Б");
	юТест.ПроверитьЛожь(НайденнаяСтрока = Неопределено, "Строка должна быть найдена по второй из колонок");

	НайденнаяСтрока = ТЗ.Найти("ЗначениеВ", "А, Б");
	юТест.ПроверитьИстину(НайденнаяСтрока = Неопределено, "Значение из колонки В не должно найтись при поиске по А и Б");

	// Поиск без указания колонок — по всем
	НайденнаяСтрока = ТЗ.Найти("ЗначениеВ");
	юТест.ПроверитьЛожь(НайденнаяСтрока = Неопределено, "Строка должна быть найдена при поиске по всем колонкам");
...
Процедура ТестДолжен_НайтиПоНесколькимКолонкамСЧастичнымИндексом() Экспорт
	...
	юТест.ПроверитьЛожь(НайденнаяСтрока = Неопределено, "Должна находиться строка по совпадению в колонке «Товар»");
	юТест.ПроверитьРавенство(НайденнаяСтрока, С1, "При совпадении в первой колонке раньше по порядку — первая строка");

	НайденнаяСтрока = ТЗ.Найти("Синий", "Товар, Цвет");
	юТест.ПроверитьИстину(НайденнаяСтрока = Неопределено, "Нет совпадений ни в одной из колонок");
	...
	НайденнаяСтрока = ТЗ.Найти("Синий", "Товар, Цвет");
	юТест.ПроверитьЛожь(НайденнаяСтрока = Неопределено, "Совпадение только в проиндексированной колонке «Цвет»");
	юТест.ПроверитьРавенство(НайденнаяСтрока, С3);
