...
Процедура ТестДолжен_ПроверитьПередачуДанныхВСкриптЧерезСтандартныйПотокВвода() Экспорт
	...
	Сообщить(СтрШаблон("Путь к oscript: %1", ПутьКОскрипт));
	...
	Сообщить(СтрШаблон("Исполняемая команда 1: %1", ИсполняемаяКоманда));
	...
	Сообщить(СтрШаблон("Исполняемая команда 2: %1", ИсполняемаяКоманда));
