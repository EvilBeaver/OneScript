...
Функция ПолучитьСписокТестов(ЮнитТестирование) Экспорт
	...
	ВсеТесты.Добавить("ТестДолжен_ПроверитьЗаписьТипаДокумента_4Параметра");
	ВсеТесты.Добавить("ТестДолжен_ПроверитьЗаписьТипаДокумента_3Параметра");
	ВсеТесты.Добавить("ТестДолжен_ПроверитьЗаписьТипаДокумента_4ПараметраСОшибкой");
	ВсеТесты.Добавить("ТестДолжен_ПроверитьЗаписьТипаДокумента_ОшибкаПослеНачала");
	ВсеТесты.Добавить("ТестДолжен_ПроверитьЗаписьТипаДокумента_ОшибкаИмени");
...
Процедура ТестДолжен_ПроверитьЗаписьТипаДокумента_4Параметра() Экспорт
	Варианты = Новый Массив();
	Варианты.Добавить("А,,,:<!DOCTYPE А>");
	//Варианты.Добавить("А,B,,:");
	Варианты.Добавить("А,,C,:<!DOCTYPE А SYSTEM ""C"">");
	Варианты.Добавить("А,B,C,:<!DOCTYPE А PUBLIC ""B"" ""C"">");
	Варианты.Добавить("А,,,D:<!DOCTYPE А[D]>");
	//Варианты.Добавить("А,B,,D:");
	Варианты.Добавить("А,,C,D:<!DOCTYPE А SYSTEM ""C""[D]>");
	Варианты.Добавить("А,B,C,D:<!DOCTYPE А PUBLIC ""B"" ""C""[D]>");

	ЗаписьXML = Новый ЗаписьXML;
	Для Каждого Пара Из Варианты Цикл
		АргРез = СтрРазделить(Пара,":");
		Арг = СтрРазделить(АргРез[0],",");
		
		ЗаписьXML.УстановитьСтроку();
		ЗаписьXML.ЗаписатьТипДокумента(Арг[0],Арг[1],Арг[2],Арг[3]);
		Результат = ЗаписьXML.Закрыть();
		юТест.ПроверитьРавенство(АргРез[1], Результат, "Ошибка для парамеров "+СтрСоединить(Арг,","));
	КонецЦикла;
	
КонецПроцедуры

Процедура ТестДолжен_ПроверитьЗаписьТипаДокумента_3Параметра() Экспорт
	Варианты = Новый Массив();
	Варианты.Добавить("А,,:<!DOCTYPE А>");
	Варианты.Добавить("А,B,:<!DOCTYPE А SYSTEM ""B"">");
	Варианты.Добавить("А,,C:<!DOCTYPE А[C]>");
	Варианты.Добавить("А,B,C:<!DOCTYPE А SYSTEM ""B""[C]>");
	...
	Для Каждого Пара Из Варианты Цикл
		...
		ЗаписьXML.УстановитьСтроку();
		ЗаписьXML.ЗаписатьТипДокумента(Арг[0],Арг[1],Арг[2]);
		Результат = ЗаписьXML.Закрыть();
		юТест.ПроверитьРавенство(АргРез[1], Результат, "Ошибка для парамеров "+СтрСоединить(Арг,","));
	КонецЦикла;
...
Процедура ТестДолжен_ПроверитьЗаписьТипаДокумента_4ПараметраСОшибкой() Экспорт
	ЗаписьXML = Новый ЗаписьXML;
	ЗаписьXML.УстановитьСтроку();

	БылоИсключение = Ложь;
	Попытка
		ЗаписьXML.ЗаписатьТипДокумента("A","B",,"D");
	Исключение
		Ошибка = ИнформацияОбОшибке().Описание;
		юТест.ПроверитьНеРавенство(Найти(Ошибка,"Отсутствует системный идентификатор"), 0, "Неверный вид ошибки: "+Ошибка);
		БылоИсключение = Истина;
	КонецПопытки;
	ЗаписьXML.Закрыть();
	
	юТест.ПроверитьИстину(БылоИсключение, "ЗаписьТипаДокумента_4ПараметраСОшибкой");
КонецПроцедуры

Процедура ТестДолжен_ПроверитьЗаписьТипаДокумента_ОшибкаПослеНачала() Экспорт
	...
	Исключение
		Ошибка = ИнформацияОбОшибке().Описание;
		юТест.ПроверитьНеРавенство(Найти(Ошибка,"Ошибочный порядок записи XML"), 0, "Неверный вид ошибки: "+Ошибка);
		БылоИсключение = Истина;
	КонецПопытки;
	ЗаписьXML.Закрыть();
	
	юТест.ПроверитьИстину(БылоИсключение, "ЗаписьТипаДокумента_ОшибкаПослеНачала");
КонецПроцедуры

Процедура ТестДолжен_ПроверитьЗаписьТипаДокумента_ОшибкаИмени() Экспорт
	...
	Исключение
		Ошибка = ИнформацияОбОшибке().Описание;
		юТест.ПроверитьНеРавенство(Найти(Ошибка,"Неверное значение аргумента номер 1"), 0, "Неверный вид ошибки: "+Ошибка);
		БылоИсключение = Истина;
	КонецПопытки;
	ЗаписьXML.Закрыть();
	
	юТест.ПроверитьИстину(БылоИсключение, "ТестДолжен_ПроверитьЗаписьТипаДокумента_ОшибкаИмени");
КонецПроцедуры
