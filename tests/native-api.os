...
Процедура ТестДолжен_ПроверитьОбменПараметров() Экспорт
	...
	// Обмен строковых out-параметров
	А = "первый";
	Б = "второй";
	...
	// Обмен числовых out-параметров
	ЧислоА = 1;
	ЧислоБ = 2;
	ВнешняяКомпонента.ОбменПараметров(ЧислоА, ЧислоБ);
	юТест.ПроверитьРавенство(ЧислоА, 2);
	юТест.ПроверитьРавенство(ЧислоБ, 1);

	// Возвращаемое значение функции не ломается обратным чтением параметров
	Параметр = Ложь;
	...
	// Пропущенный параметр по умолчанию — без исключений
	юТест.ПроверитьРавенство(ВнешняяКомпонента.ПараметрПоУмолчанию(), Истина);
