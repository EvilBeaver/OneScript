...
Процедура ТестДолжен_ПроверитьПодключениеВнешнейКомпонентыDLL() Экспорт
	...
	Иначе
		...
		Это32бит = ПодключитьВнешнююКомпоненту(ПутьБиблиотеки + "build32/libAddInNativeLin32.so", Идентификатор, ТипВнешнейКомпоненты.Native);
		Это64бит = ПодключитьВнешнююКомпоненту(ПутьБиблиотеки + "build64/libAddInNativeLin64.so", Идентификатор, ТипВнешнейКомпоненты.Native);
	КонецЕсли;
