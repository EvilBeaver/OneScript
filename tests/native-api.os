...
Процедура ТестДолжен_ПроверитьПодключениеВнешнейКомпонентыZIP() Экспорт
	...
	Иначе
		Каталог = ТекущийСценарий().Каталог + "../built/tmp/tests/";
		МасивФайлов.Добавить("AddInNativeLin32.so");
		МасивФайлов.Добавить("AddInNativeLin64.so");
	КонецЕсли;
...
Процедура ТестДолжен_ПроверитьПодключениеВнешнейКомпонентыDLL() Экспорт
	...
	Иначе
		ПутьБиблиотеки = ТекущийСценарий().Каталог + "../built/tmp/tests/";
		Это32бит = ПодключитьВнешнююКомпоненту(ПутьБиблиотеки + "AddInNativeLin32.so", Идентификатор, ТипВнешнейКомпоненты.Native);
		Это64бит = ПодключитьВнешнююКомпоненту(ПутьБиблиотеки + "AddInNativeLin64.so", Идентификатор, ТипВнешнейКомпоненты.Native);
	КонецЕсли;
