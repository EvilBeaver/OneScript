...
Функция ПолучитьСписокТестов(ЮнитТестирование) Экспорт
	...
	ВсеТесты.Добавить("ТестДолжен_ПроверитьСортировкуПоЗначениюРазныхТипов");
	...
	ВсеТесты.Добавить("ТестДолжен_ПроверитьСвойствоТипЗначения");
	ВсеТесты.Добавить("ТестДолжен_ПроверитьСвойствоДоступныеЗначения_Без_ТипЗначения");
	ВсеТесты.Добавить("ТестДолжен_ПроверитьСвойствоДоступныеЗначения_С_ТипЗначения");
	ВсеТесты.Добавить("ТестДолжен_ПроверитьПреобразованиеСпискаВСтроку");
	ВсеТесты.Добавить("ТестДолжен_ПроверитьМетодПолучитьИдентификатор");
	ВсеТесты.Добавить("ТестДолжен_ПроверитьМетодЗагрузитьЗначенияСПриведениемЗначений");
	Возврат ВсеТесты;
...
Процедура ТестДолжен_ПроверитьСортировкуПоЗначениюРазныхТипов() Экспорт
	СЗ = Новый СписокЗначений;

	СЗ.Добавить(Тип("Строка"));
	СЗ.Добавить("Стр2");
	СЗ.Добавить("СтрЪ");
	СЗ.Добавить();
	СЗ.Добавить(Новый Структура);
	СЗ.Добавить(Новый Массив);
	СЗ.Добавить(null);
	СЗ.Добавить(Тип("Булево")); 
	СЗ.Добавить(Тип("Массив")); 
	СЗ.Добавить(Тип("ЗаписьXML")); 
	СЗ.Добавить(Новый ЧтениеJSON);
	СЗ.Добавить(Новый ЗаписьXML);

	СЗ.СортироватьПоЗначению();
	
	юТест.ПроверитьРавенство(СЗ[0].Значение, Неопределено);
	юТест.ПроверитьРавенство(СЗ[1].Значение, null);
	юТест.ПроверитьРавенство(СЗ[2].Значение, "Стр2");
	юТест.ПроверитьРавенство(СЗ[3].Значение, "СтрЪ");
	юТест.ПроверитьТип(СЗ[4].Значение, Тип("ЗаписьXML"));
	юТест.ПроверитьТип(СЗ[5].Значение, Тип("Массив"));
	юТест.ПроверитьТип(СЗ[6].Значение, Тип("Структура"));
	юТест.ПроверитьТип(СЗ[7].Значение, Тип("ЧтениеJSON"));
	юТест.ПроверитьРавенство(СЗ[8].Значение, Тип("Булево"));
	юТест.ПроверитьРавенство(СЗ[9].Значение, Тип("ЗаписьXML"));
	юТест.ПроверитьРавенство(СЗ[10].Значение, Тип("Массив"));
	юТест.ПроверитьРавенство(СЗ[11].Значение, Тип("Строка"));
КонецПроцедуры
...
Процедура ТестДолжен_ПроверитьСортировкуНеупорядочиваемыхТипов() Экспорт
	...
	СЗ.Добавить(Неопределено);
	СЗ.Добавить(Неопределено);

	СЗ.СортироватьПоЗначению(); // не должно упасть

	юТест.ПроверитьРавенство(СЗ[0].Значение, Неопределено);
	юТест.ПроверитьРавенство(СЗ[1].Значение, Неопределено);
КонецПроцедуры
...
Процедура ТестДолжен_ПроверитьПреобразованиеЭлементаСпискаВСтроку() Экспорт
	...
	СЗ.Добавить(3, " ");
	СЗ.Добавить(Неопределено);
	СЗ.Добавить(Неопределено,"(неопр.)");
	СЗ.Добавить(NULL);
	СЗ.Добавить(NULL,"(null)");

	юТест.ПроверитьРавенство("0", Строка(СЗ[0]), "Строка, без указания");
	юТест.ПроверитьРавенство("", СЗ[0].Представление, "Представление, без указания");
	юТест.ПроверитьРавенство("1", Строка(СЗ[1]), "Строка, с пустой");
	юТест.ПроверитьРавенство("", СЗ[1].Представление, "Представление, с пустой");
	юТест.ПроверитьРавенство("2", Строка(СЗ[2]), "Строка, Неопределено");
	юТест.ПроверитьРавенство("", СЗ[2].Представление, "Представление, Неопределено");
	юТест.ПроверитьРавенство(" ", Строка(СЗ[3]), "Строка, пробел");
	юТест.ПроверитьРавенство(" ", СЗ[3].Представление, "Представление, пробел");
	
	юТест.ПроверитьРавенство("<Пустое значение>", Строка(СЗ[4]), "Неопределено, без указания");
	юТест.ПроверитьРавенство("", СЗ[4].Представление, "Неопределено, без представления");
	юТест.ПроверитьРавенство("(неопр.)", Строка(СЗ[5]), "Неопределено, с указанием");
	юТест.ПроверитьРавенство("(неопр.)", СЗ[5].Представление, "Неопределено, с представлением");

	юТест.ПроверитьРавенство("<Пустое значение>", Строка(СЗ[6]), "NULL, без указания");
	юТест.ПроверитьРавенство("", СЗ[6].Представление, "NULL, без представления");
	юТест.ПроверитьРавенство("(null)", Строка(СЗ[7]), "NULL, с указанием");
	юТест.ПроверитьРавенство("(null)", СЗ[7].Представление, "NULL, с представлением");


КонецПроцедуры

Процедура ТестДолжен_ПроверитьСвойствоТипЗначения() Экспорт 
	Описание = Новый ОписаниеТипов("Число", Новый КвалификаторыЧисла(2));
	СЗ = Новый СписокЗначений;
	СЗ.ТипЗначения = Описание;
	СЗ.Добавить(12);
	СЗ.Добавить(22);
	СЗ.Добавить(122);
	СЗ.Добавить("23");
	...
	юТест.ПроверитьРавенство(12, СЗ[0].Значение);
	юТест.ПроверитьРавенство(22, СЗ[1].Значение);
	юТест.ПроверитьРавенство(99, СЗ[2].Значение);
	юТест.ПроверитьТип(СЗ[3].Значение,"Число");
	юТест.ПроверитьРавенство(23, СЗ[3].Значение);
	юТест.ПроверитьРавенство(0, СЗ[4].Значение);
КонецПроцедуры

Процедура ТестДолжен_ПроверитьСвойствоДоступныеЗначения_Без_ТипЗначения() Экспорт 
	ДЗ = Новый СписокЗначений; 
	ДЗ.Добавить(11);
	ДЗ.Добавить(12);
	ДЗ.Добавить(99);

	СЗ = Новый СписокЗначений;
	СЗ.ДоступныеЗначения = ДЗ;
	СЗ.Добавить(12);
	СЗ.Добавить(22);
	СЗ.Добавить("11");

	юТест.ПроверитьТип(СЗ[0].Значение,"Число");
	юТест.ПроверитьРавенство(12, СЗ[0].Значение, "Допустимое значение");
	юТест.ПроверитьРавенство( Неопределено, СЗ[1].Значение, "Недопустимое значение");
	юТест.ПроверитьРавенство( Неопределено, СЗ[2].Значение, "Без приведения типа");
КонецПроцедуры

Процедура ТестДолжен_ПроверитьСвойствоДоступныеЗначения_С_ТипЗначения() Экспорт
	ДЗ = Новый СписокЗначений; 
	ДЗ.Добавить(11);
	ДЗ.Добавить(12);
	ДЗ.Добавить(99); 
	...
	СЗ.Добавить(12);
	СЗ.Добавить(22);
	СЗ.Добавить("11"); // приведение типа
	СЗ.Добавить(123); // переполнение

	юТест.ПроверитьТип(СЗ[0].Значение,"Число");
	юТест.ПроверитьРавенство(12, СЗ[0].Значение, "Допустимое значение");
	юТест.ПроверитьРавенство( 0, СЗ[1].Значение, "Недопустимое значение с приведением типа");
	юТест.ПроверитьТип(СЗ[2].Значение,"Число");
	юТест.ПроверитьРавенство( 11, СЗ[2].Значение, "Допустимое значение с приведением типа");
	юТест.ПроверитьРавенство( 99, СЗ[3].Значение, "Приведение с переполнением и допустимое значение");
КонецПроцедуры

Процедура ТестДолжен_ПроверитьПреобразованиеСпискаВСтроку() Экспорт

	СЗ = Новый СписокЗначений;
	СЗ.Добавить(11);
	СЗ.Добавить(12,"двенадцать");
	СЗ.Добавить("абв");
	СЗ.Добавить("фыв","");
	СЗ.Добавить("пробел"," ");
	СЗ.Добавить("йцу","Йцу!");
	...
	юТест.ПроверитьРавенство(Строка, "11; двенадцать; абв; фыв;  ; Йцу!; <Пустое значение>; (неопр.)");
КонецПроцедуры

Процедура ТестДолжен_ПроверитьМетодПолучитьИдентификатор() Экспорт
	СЗ = Новый СписокЗначений;
	...
	юТест.ПроверитьРавенство(-4, СЗ[0].Значение);
	юТест.ПроверитьРавенство(3, СЗ[0].ПолучитьИдентификатор());
	
	юТест.ПроверитьРавенство(-2, СЗ[1].Значение);
	юТест.ПроверитьРавенство(1, СЗ[1].ПолучитьИдентификатор());
	
	юТест.ПроверитьРавенство(0, СЗ[2].Значение);
	юТест.ПроверитьРавенство(4, СЗ[2].ПолучитьИдентификатор());
	
	юТест.ПроверитьРавенство(1, СЗ[3].Значение);
	юТест.ПроверитьРавенство(0, СЗ[3].ПолучитьИдентификатор());

КонецПроцедуры 

Процедура ТестДолжен_ПроверитьМетодЗагрузитьЗначенияСПриведениемЗначений() Экспорт
	Массив = Новый Массив;
	Массив.Добавить(12);
	Массив.Добавить(123);
	...
	юТест.ПроверитьРавенство(2, СЗ.Количество());
	юТест.ПроверитьРавенство(12, СЗ[0].Значение);
	юТест.ПроверитьРавенство(99, СЗ[1].Значение);
КонецПроцедуры
