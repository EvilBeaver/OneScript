...
Процедура Тест_Должен_ПроверитьСохранениеПотокаПослеЗакрытия() Экспорт
	...
	Поток.Записать(ПолучитьБуферДвоичныхДанныхИзСтроки("БДД"),0,2); 
	...
	Поток.Записать(ПолучитьБуферДвоичныхДанныхИзСтроки("БДД"),2,2); 
	РезПотока =  Поток.ЗакрытьИПолучитьДвоичныеДанные();
	
	юТест.ПроверитьРавенство("123", РезЗаписи);
	юТест.ПроверитьРавенство("D0 91 D0 94", Строка(РезПотока));
КонецПроцедуры
