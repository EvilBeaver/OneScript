...
Функция ПолучитьСписокТестов(Тестирование) Экспорт
	...
	СписокТестов.Добавить("Тест_Должен_ЧитатьДатуВФорматеISOПриИспользованииПрочитатьJSONСИменамиСвойствДат");
...
Процедура Тест_Должен_ЧитатьДатуВФорматеISOПриИспользованииПрочитатьJSONСИменамиСвойствДат() Экспорт

	СтрокаJSON = "{ ""lastSignInTime"": ""2026-05-13T22:51:46.230Z"" }";

	ИменаСвойств = Новый Массив;
	ИменаСвойств.Добавить("lastSignInTime");

	ЧтениеJSON = Новый ЧтениеJSON;
	ЧтениеJSON.УстановитьСтроку(СтрокаJSON);
	Объект = ПрочитатьJSON(ЧтениеJSON, Ложь, ИменаСвойств, ФорматДатыJSON.ISO);
	ЧтениеJSON.Закрыть();

	Ожидание = МестноеВремя(Дата(2026, 5, 13, 22, 51, 46));

	юТест.ПроверитьРавенство(Тип("Дата"), ТипЗнч(Объект.lastSignInTime));
	юТест.ПроверитьРавенство(Ожидание, Объект.lastSignInTime);
	юТест.ПроверитьРавенство(0, Объект.lastSignInTime - Дата(Год(Объект.lastSignInTime), Месяц(Объект.lastSignInTime), День(Объект.lastSignInTime), Час(Объект.lastSignInTime), Минута(Объект.lastSignInTime), Секунда(Объект.lastSignInTime)));
