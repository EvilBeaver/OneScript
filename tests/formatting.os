...
Процедура ТестДолжен_ПроверитьФорматированиеПредставлениеНуля() Экспорт
	...
	юТест.ПроверитьРавенство("", Формат(0));
	юТест.ПроверитьРавенство("", Формат(0, ""));
...
Процедура ТестДолжен_ПроверитьФорматированиеПустаяДата() Экспорт
	...
	юТест.ПроверитьРавенство("01.01.0001", Формат(ПустаяДата, "ДФ='dd.MM.yyyy'; ДП=''"),
		"Есть ДП, пустой, без локализации");
	
	// в оригинале перед буквой `г` должен стоять неразрывный пробел
	юТест.ПроверитьРавенство("1 января 0001 г.", Формат(ПустаяДата, "Л='ru_RU'; ДЛФ='DD'; ДП=''"),
		"Есть ДП, пустой, с локализацией");
	
	юТест.ПроверитьРавенство("", Формат(ПустаяДата),
		"параметр Формат опущен");

	юТест.ПроверитьРавенство("", Формат(ПустаяДата, ""),
		"параметр Формат пустой.");
	
	юТест.ПроверитьРавенство("", Формат(ПустаяДата, "ДФ=гггг.ДД.мм"),
		"Нет ДП, есть формат");
	
	юТест.ПроверитьРавенство("", Формат(ПустаяДата, "ДЛФ='DD'"),
		"Нет ДП, есть формат локальный");
	
	юТест.ПроверитьРавенство("", Формат(ПустаяДата, "ДП=''"),
		"Есть ДП, пустой, без формата");
