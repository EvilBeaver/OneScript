Перем юТест;

Функция ПолучитьСписокТестов(ЮнитТестирование) Экспорт
	
	юТест = ЮнитТестирование;
	
	ВсеТесты = Новый Массив;
	
	ВсеТесты.Добавить("ТестДолжен_УбедитьсяЧтоВыводитсяВерсия");
	
	Возврат ВсеТесты;
КонецФункции

Функция ПутьОСкрипт()

	Возврат "dotnet """ + ОбъединитьПути(КаталогПрограммы(), "oscript.dll") + """";
	
КонецФункции


Процедура ТестДолжен_УбедитьсяЧтоВыводитсяВерсия() Экспорт

	Перем Процесс;
	
	СистемнаяИнформация = Новый СистемнаяИнформация;

	ПроверяемыеКлючи = Новый Массив;
	ПроверяемыеКлючи.Добавить("-version");
	ПроверяемыеКлючи.Добавить("-v");

	Для Каждого мПроверяемыйКлюч Из ПроверяемыеКлючи Цикл
		
		Процесс = СоздатьПроцесс(ПутьОСкрипт() + " " + мПроверяемыйКлюч, , Истина);
		Процесс.Запустить();
		Процесс.ОжидатьЗавершения();
		
		// Прочитать() возвращает вывод как есть, включая завершающий перевод строки
		ДанныеВывода = СокрЛП(Процесс.ПотокВывода.Прочитать());

		юТест.ПроверитьРавенство(СистемнаяИнформация.Версия, ДанныеВывода, "Версия вывода не совпадает с системной информацией!");

	КонецЦикла;

КонецПроцедуры


