...
Процедура ТестДолжен_ВызватьМетод_СтрШаблон_СПередачейПеременныхНеопределено() Экспорт
	
	Перем п1,п2,п3,п4,п5,п6;
	
	п2 = "Значение";
	
	Результат = СтрШаблон("тест %1 тест %2 тест %3", п1,п2,п3,п4,п5,п6);
	юТест.ПроверитьРавенство("тест  тест Значение тест ", Результат);
