...
namespace OneScript.StandardLibrary
{
    ...
    public class StandardGlobalContext : GlobalContextBase<StandardGlobalContext>
    {
        ...
        /// Идентификаторы выдаются последовательно, начиная с единицы, и в пределах запущенного
        /// движка не повторяются, пока счётчик не исчерпает диапазон Int32. После этого значения
        /// начинают выдаваться заново и могут совпасть с идентификатором долгоживущей единицы
        /// исполнения, поэтому хранилища, живущие всё время работы движка, должны освобождать
        /// записи завершившихся единиц исполнения, а не накапливать их.
        ///
