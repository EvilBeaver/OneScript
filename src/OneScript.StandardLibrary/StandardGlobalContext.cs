...
namespace OneScript.StandardLibrary
{
    ...
    public class StandardGlobalContext : GlobalContextBase<StandardGlobalContext>
    {
        ...
        /// <summary>
        /// Возвращает поток исполнения, в котором выполняется текущий код.
        ///
        /// Отдельным потоком исполнения является каждая независимая единица исполнения bsl-кода:
        /// основной скрипт, каждое фоновое задание и каждый обрабатываемый запрос веб-сервера.
        /// Для одной единицы исполнения всегда возвращается один и тот же объект, поэтому его
        /// свойство Данные можно использовать как набор thread-local переменных.
        ///
        /// В отличие от идентификатора фонового задания, поток исполнения определён во всех
        /// контекстах, в том числе при обработке запросов веб-сервера, где фоновое задание
        /// отсутствует.
        ///
        /// Поток исполнения не наследуется: фоновое задание, запущенное из текущего потока
        /// исполнения, получит собственный поток с собственными данными.
        /// </summary>
        /// <returns>ПотокИсполнения. Поток исполнения текущего кода.</returns>
        [ContextMethod("ТекущийПоток", "CurrentThread")]
